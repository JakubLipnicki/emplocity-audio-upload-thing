...
from django.conf import settings  # To access Django project settings (PAYU_*, YOUR_APP_BASE_URL)
from django.core.cache import cache  # For caching the PayU access token

# Unique cache key for storing the PayU access token.
PAYU_ACCESS_TOKEN_CACHE_KEY = 'payu_access_token_v3'  # Versioned to avoid potential old cache conflicts
...
def get_payu_access_token():
    ...
    try:
        ...
        if not access_token:
            print(f"PAYU_SERVICE_ERROR: 'access_token' not found in PayU OAuth response. Response: {token_data}")
            return None
        ...
        print(f"PAYU_SERVICE_INFO: Successfully fetched and cached PayU OAuth token. Expires in: {cache_timeout}s")
        return access_token

    except requests.exceptions.HTTPError as e:
        print(f"PAYU_SERVICE_ERROR: HTTPError during OAuth token request: {e.response.status_code} - {e.response.text}")
...
def create_payu_order_api_call(order_payload_dict):
    ...
    if not access_token:
        print("PAYU_SERVICE_ERROR: Cannot create PayU order - failed to obtain access token.")
    ...
    if not api_url:
        ...
        return {'error': 'Configuration error', 'details': 'PayU API Order URL is not set.'}
    ...
    try:
        ...
        response = requests.post(api_url, json=order_payload_dict, headers=headers, timeout=15, allow_redirects=False)

        print(f"PAYU_SERVICE_INFO: Received response from PayU. Status: {response.status_code}")
        ...
        try:
            ...
            response_data = {
                'error_raw_response': response.text[:500]}  # Store a snippet of non-JSON response for debugging.
            print(
        ...
        if response.status_code not in [200, 201, 302]:  # Consider these as "successful" or "redirect" statuses.
    ...
    except requests.exceptions.HTTPError as e:  # Handles 4xx/5xx from response.raise_for_status() if it were used for all codes
        ...
        return {'error': 'HTTPError', 'status_code_received_from_payu': e.response.status_code,
