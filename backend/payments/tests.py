...
class AccountRegistrationTests(APITestCase):
    ...
    def test_register_user_success(self):
        response = self.client.post(self.register_url, self.user_data_valid, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(User.objects.filter(email=self.user_data_valid['email']).exists())
    ...
    def test_register_user_duplicate_email(self):
        ...
        response = self.client.post(self.register_url, self.user_data_duplicate_email, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(User.objects.count(), 1)
        self.assertIn('email', response.data)

    def test_register_user_missing_email(self):
        response = self.client.post(self.register_url, self.user_data_missing_email, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('email', response.data)

    def test_register_user_missing_password(self):
        response = self.client.post(self.register_url, self.user_data_missing_password, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class AccountLoginAndUserViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.login_data_active_correct = {'email': self.active_user_email, 'password': self.active_user_password}
        self.login_data_active_wrong_pass = {'email': self.active_user_email, 'password': 'wrongpassword123'}
        self.login_data_nonexistent_user = {'email': 'iamnotregistered@example.com', 'password': 'anypassword'}
        self.login_data_inactive_user = {'email': self.inactive_user_email, 'password': self.inactive_user_password}

    def test_login_active_user_success(self):
        response = self.client.post(self.login_url, self.login_data_active_correct, format='json')
    ...
    def test_login_active_user_wrong_password(self):
        response = self.client.post(self.login_url, self.login_data_active_wrong_pass, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('detail', response.data)
        self.assertEqual(str(response.data['detail']), "Incorrect password!")

    def test_login_nonexistent_user(self):
        response = self.client.post(self.login_url, self.login_data_nonexistent_user, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('detail', response.data)
        self.assertEqual(str(response.data['detail']), "User not found!")

    def test_login_inactive_user(self):
        response = self.client.post(self.login_url, self.login_data_inactive_user, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('detail', response.data)
        self.assertEqual(str(response.data['detail']), "Account not activated. Please check your email.")
    ...
    def test_logout_user_success(self):
        login_response = self.client.post(self.login_url, self.login_data_active_correct, format='json')
    ...
    def test_refresh_token_success(self):
        ...
        self.assertEqual(refresh_response.data['message'], "Access token refreshed successfully")
        ...
        if original_access_token_cookie:
            self.assertNotEqual(new_access_token_cookie.value, original_access_token_cookie.value)
        self.assertTrue(new_access_token_cookie['httponly'])
        self.assertEqual(bool(new_access_token_cookie.get('secure')), bool(settings.SESSION_COOKIE_SECURE))
        self.assertEqual(new_access_token_cookie['samesite'], 'Lax')
        self.assertEqual(int(new_access_token_cookie['max-age']), settings.ACCESS_TOKEN_EXPIRATION_TIME_HOURS * 60 * 60)

    def test_refresh_token_no_refresh_cookie(self):
        response = self.client.post(self.refresh_token_url, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('detail', response.data)
        self.assertEqual(str(response.data['detail']), "Refresh token not found in cookies.")
    ...
    def test_refresh_token_invalid_or_expired_refresh_cookie(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('detail', response.data)
        self.assertTrue("Invalid or expired refresh token" in str(response.data['detail']))
...
class PaymentInitiationTests(APITestCase):
    ...
    def _login_user(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, "Pre-test login failed.")
        self.assertTrue(self.client.cookies.get('access_token'), "Access token cookie not set after login.")

    def test_initiate_payment_unauthenticated(self):
        response = self.client.post(self.initiate_payment_url, self.valid_payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn('detail', response.data)
        self.assertEqual(str(response.data['detail']), "Nie podano danych uwierzytelniających.")
    ...
    def test_initiate_payment_success_authenticated(self, mock_create_payu_order):
        ...
        self.assertEqual(response.data['redirectUri'], mock_payu_response['redirectUri'])
    ...
    def test_initiate_payment_missing_amount_authenticated(self):
        self._login_user()
        response = self.client.post(self.initiate_payment_url, self.payload_missing_amount, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_initiate_payment_invalid_amount_zero_authenticated(self):
        self._login_user()
        response = self.client.post(self.initiate_payment_url, self.payload_invalid_amount_zero, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('error', response.data)
    ...
    def test_initiate_payment_invalid_amount_negative_authenticated(self):
        self._login_user()
        response = self.client.post(self.initiate_payment_url, self.payload_invalid_amount_negative, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('error', response.data)
        self.assertTrue('amount' in response.data['error'].lower())
    ...
    def test_initiate_payment_missing_description_authenticated(self):
        self._login_user()
        response = self.client.post(self.initiate_payment_url, self.payload_missing_description, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('error', response.data)
    ...
    def test_initiate_payment_payu_api_call_fails(self, mock_create_payu_order):
        ...
        self.assertIn('error', response.data)
        self.assertEqual(response.data['error'], 'Failed to initiate payment with PayU.')
        self.assertIn('details', response.data)
        self.assertEqual(response.data['details'], 'Simulated connection timeout')
        db_order = PaymentOrder.objects.filter(user=self.user).order_by('-created_at').first()
...
class PaymentNotificationTests(APITestCase):
    ...
    def setUp(self):
        ...
        if not self.payu_signature_key:  # Fallback, jeśli nie ma w settings (nie powinno się zdarzyć)
            self.payu_signature_key = "test_signature_key_fallback_for_tests"
            print("WARNING: Using fallback PAYU_SIGNATURE_KEY in tests. Ensure it's set in settings via .env")

    def _prepare_notification_data(self, payu_status, ext_order_id=None, payu_order_id=None):
        ...
        payu_order_id = payu_order_id or "PAYU_ORDER_ID_NOTIF_TEST"  # Przykładowe ID PayU
    ...
    def test_process_completed_notification_success(self):
        ...
        signature = generate_payu_signature(notification_body_bytes, self.payu_signature_key)
    ...
    def test_process_notification_invalid_signature(self):
        ...
        invalid_signature = generate_payu_signature(notification_body_bytes, "wrong_key_for_signature")

        headers = {
            'HTTP_OPENPAYU_SIGNATURE': f"signature={invalid_signature};algorithm=SHA-256"
        }
        response = self.client.post(
            self.notify_url, data=notification_body_bytes,
            content_type='application/json', **headers
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.order.refresh_from_db()
        self.assertEqual(self.order.status, PaymentOrder.Status.PENDING)  # Status nie powinien się zmienić
    ...
    def test_process_canceled_notification(self):
        ...
        notification_body_bytes = json.dumps(notification_dict).encode('utf-8')
        signature = generate_payu_signature(notification_body_bytes, self.payu_signature_key)
        headers = {'HTTP_OPENPAYU_SIGNATURE': f"signature={signature};algorithm=SHA-256"}
    ...
    def test_process_notification_order_not_found(self):
        ...
        notification_dict = self._prepare_notification_data(payu_status="COMPLETED",
                                                            ext_order_id="NON_EXISTENT_ORDER_ID")
        notification_body_bytes = json.dumps(notification_dict).encode('utf-8')
        signature = generate_payu_signature(notification_body_bytes, self.payu_signature_key)
        headers = {'HTTP_OPENPAYU_SIGNATURE': f"signature={signature};algorithm=SHA-256"}
