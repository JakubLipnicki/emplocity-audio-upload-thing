...
class PaymentOrder(models.Model):
    ...
    class Status(models.TextChoices):
        ...
        PROCESSING = (
            "PROCESSING",
            "Processing",
        )  # Payment in progress with PayU (e.g., WAITING_FOR_CONFIRMATION).
        COMPLETED = (
            "COMPLETED",
            "Completed",
        ...
        FAILED = (
            "FAILED",
            "Failed",
        )  # Payment failed due to technical reasons or rejection.
    ...
    )  # Unique order ID sent to PayU; auto-set in save() if blank.
    payu_order_id = models.CharField(
        max_length=100, blank=True, null=True, unique=True
    )  # Order ID returned by PayU.
    ...
    )  # Order amount in the smallest currency unit (e.g., grosze for PLN).
    currency = models.CharField(
        max_length=3, default="PLN"
    )  # Transaction currency code (e.g., PLN).
    description = models.CharField(
        max_length=255
    )  # Brief order description for PayU (e.g., "Zakup ramki profilowej 'NazwaRamki'").
    ...
    )  # Could be pre-filled from user.email
    buyer_first_name = models.CharField(
        max_length=100, blank=True, null=True
    )  # Could be pre-filled from user.first_name
    buyer_last_name = models.CharField(
        max_length=100, blank=True, null=True
    )  # Could be pre-filled from user.last_name

    # --- PayU Process Related Data ---
    redirect_uri_payu = models.URLField(
        max_length=500, blank=True, null=True
    )  # Redirect URL to PayU payment gateway.

    # --- Timestamps (Automatically Managed by Django) ---
    created_at = models.DateTimeField(
        auto_now_add=True
    )  # Timestamp of order record creation.
    updated_at = models.DateTimeField(
        auto_now=True
    )  # Timestamp of last order record modification.
