...
class PaymentOrderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {  # Main section, no title
            'fields': ('id', 'status', 'amount', 'currency', 'user')  # Add 'user' if you have it linked.
        }),
        ('PayU Integration Details', {
            'fields': ('ext_order_id', 'payu_order_id', 'redirect_uri_payu', 'description')
        }),
        ('Buyer Information', {  # Information about the person making the payment.
            'fields': ('buyer_email', 'buyer_first_name', 'buyer_last_name')
        }),
        ('Timestamps', {  # Auto-managed date and time fields.
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)  # Makes this fieldset collapsible in the admin interface.
        }),
    )
    ...
    user_email_display.admin_order_field = 'user__email'  # Allows sorting by user's email.