...
def validate_audio_file_extension(value):
    ...
    if f".{ext}" not in ALLOWED_AUDIO_EXTENSIONS:
        raise ValidationError(f"Allowed audio file formats: {', '.join(ALLOWED_AUDIO_EXTENSIONS)}.")
...
class AudioFile(models.Model):
    ...
    file = models.FileField(upload_to=audio_file_upload_to, storage=S3Boto3Storage(), validators=[validate_audio_file_extension])
